  ...
  return <section className="dev-overview" aria-label="프로젝트 전체 현황">
    ...
    <div className="dev-overview-grid">{projects.map(project => {
      ...
      return <article key={project.id}><button className="dev-project-title" onClick={() => onChoose(project.id)}>{project.title}</button><p>{project.description || "프로젝트 목적을 확인해 주세요."}</p>
        {summary?.error ? <p role="alert">현황을 불러오지 못했습니다. 새로고침해 주세요.</p> : <div className="dev-project-counts">{STATES.map(([key, label]) => { const count = summary ? summary.counts[key] : "—"; return <button key={key} aria-label={`${label} ${count}건 보기`} title={`${label} 요청 보기`} onClick={() => onChoose(project.id, key)}><strong>{count}</strong></button>; })}</div>}
