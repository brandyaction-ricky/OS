    ...
    {tab === "niches" || tab === "planning" ? <>
      ...
      <section className="content-planning-layout">
        ...
        <article className="panel planning-detail niche-detail">{selected ? <>
          ...
          {showReferenceCheck ? <ContentPlanningHandoff key={`handoff:${selected.id}:${selected.version}:${profile?.id}`} source={selected} disabled={busy} onSaved={(record) => { setRecords((current) => current.map((item) => item.id === record.id ? record : item)); setNotice("제작 인계 메모를 저장했습니다. 승인·공유 상태는 변경하지 않았습니다."); }} /> : null}
