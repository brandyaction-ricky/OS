...
export function ContentRadarWorkspace() {
  ...
  return <>
    ...
    {tab === "discovery" ? <>
      ...
      <section className="panel content-data-table">
        <div className="panel-header"><div><h2>추적 채널 자동 발견</h2><p>매일 수집한 채널 영상 중 회사 아웃라이어 기준을 넘은 근거입니다.</p></div><span>{outliers.filter((record) => meta<string>(record, "discoverySource", "") === "tracked_channel").length}개</span></div>
        <div className="content-table-head"><span>영상</span><span>채널</span><span>조회</span><span>비교 기준</span></div>
        {outliers.filter((record) => meta<string>(record, "discoverySource", "") === "tracked_channel").slice(0, 12).map((record) => <a className="content-table-row" href={record.source_url || "#"} target="_blank" rel="noreferrer" key={record.id}><span><strong>{record.title}</strong><small>{new Date(meta(record, "observedAt", record.created_at)).toLocaleDateString("ko-KR")}</small></span><span>{meta(record, "channelTitle", "채널 미입력")}</span><span>{Number(meta(record, "views", record.metric_current ?? 0)).toLocaleString("ko-KR")}</span><span>{Number(meta<Record<string, unknown>>(record, "baseline", {}).ratio ?? 0).toFixed(2)}배</span></a>)}
        {!outliers.some((record) => meta<string>(record, "discoverySource", "") === "tracked_channel") ? <div className="list-empty">첫 일일 수집 뒤 회사 기준을 넘은 영상이 여기에 표시됩니다.</div> : null}
      </section>
