...
import { ArrowRight, Bell, Check, CheckCircle2, RefreshCw, UserRoundCheck, X } from "lucide-react";
...
export function DevelopmentRequestNotifications({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const refresh = useCallback(async () => {
    ...
    requestRef.current = controller; setRefreshing(true);
    try {
  ...
  const markRead = useCallback(async (ids: string[]) => {
    if (!accessToken || !ids.length || reading) return false;
    setReading(true);
    try {
      const response = await fetch("/api/v1/development-notifications", {
      ...
      setNotificationSummary((previous) => previous ? {
        ...previous,
        unread: Math.max(0, previous.unread - previous.notifications.filter((item) => ids.includes(item.id) && !item.readAt).length),
  ...
  const openNotification = async (item: DevelopmentNotificationItem) => {
    ...
    onOpenChange(false); router.push(`/knowledge/development?request=${encodeURIComponent(item.requestId)}`);
  };
  ...
  useEffect(() => {
    ...
    return () => { requestRef.current?.abort(); window.clearInterval(timer); window.removeEventListener("focus", onRefresh); window.removeEventListener("brandy-development-requests-changed", onRefresh); document.removeEventListener("visibilitychange", onRefresh); };
  }, [refresh]);

  useEffect(() => {
    if (!open) return;
    panelRef.current?.focus(); void refresh();
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape") { event.preventDefault(); onOpenChange(false); triggerRef.current?.focus(); } };
    const onPointerDown = (event: PointerEvent) => { if (event.target instanceof Node && !wrapperRef.current?.contains(event.target)) onOpenChange(false); };
  ...
  const unreadIds = notificationSummary?.notifications.filter((item) => !item.readAt).map((item) => item.id) ?? [];
  ...
  return <div className="development-notifications" ref={wrapperRef} onBlur={(event) => { if (event.relatedTarget instanceof Node && !event.currentTarget.contains(event.relatedTarget)) onOpenChange(false); }}>
    ...
    {open ? <section id={panelId} ref={panelRef} className="development-notifications-panel" role="dialog" aria-labelledby={`${panelId}-title`} tabIndex={-1}>
      ...
      : <>
        ...
        <div className="development-notifications-count"><span>{canManage ? "미완료 요청" : "내 미완료 요청"}</span><strong>{pending}<small>건</small></strong></div>
        <div className="development-notifications-statuses">{OPEN_STATUSES.map(({ id, label }) => <Link key={id} href={`/knowledge/development?status=${id}${canManage ? "" : "&scope=mine"}`} onClick={() => onOpenChange(false)}><span>{label}</span><strong>{summary.counts[id]}</strong></Link>)}</div>
        {recentRequests.length ? <div className="development-notifications-list"><p>{canManage ? "최근 요청 · 접수 우선" : "내 최근 요청"}</p>{recentRequests.map((request) => <Link key={request.id} href={`/knowledge/development?request=${encodeURIComponent(request.id)}`} onClick={() => onOpenChange(false)}><span className={`development-notifications-status is-${request.status}`}>{statusLabel(request.status)}</span><strong>{request.title}</strong><ArrowRight size={14} aria-hidden="true" /></Link>)}</div>
        : <div className="development-notifications-state"><CheckCircle2 size={22} />{canManage ? pending ? "전체 요청에서 미완료 항목을 확인해 주세요." : "미완료 요청이 없습니다." : summary.total ? "전체 요청에서 진행 상황을 확인해 주세요." : "아직 남긴 수정 요청이 없습니다."}</div>}
      </>}
      <footer><span>{demo ? "운영 연결 필요" : !ready ? "로그인 확인 중" : refreshing ? "알림 확인 중…" : "접속 중 1분마다 갱신"}</span><Link href={listHref} onClick={() => onOpenChange(false)}>{canManage ? "개발 관리 열기" : "내 요청 모두 보기"}<ArrowRight size={14} /></Link></footer>
    </section> : null}
  </div>;
}
