...
export function SystemOneSandbox() {
  ...
  return <div className="linear-shell"><main className={styles.shell}>
    <header className={`topbar ${styles.header}`}>
      <div className={styles.brand}><span className="brand-mark">BA</span><span>브랜디 OS <span className={styles.brandDivider}>/</span> 공통 판단</span></div>
      <span className={styles.mockBadge}><FlaskConical size={14} /> 모의 검토 · 저장되지 않음</span>
    </header>
    <section className={`page-header page-title-group ${styles.hero}`}>
      <span className={styles.eyebrow}>SYSTEM ONE · 로컬 검수</span>
      <h1>공통 판단</h1>
    ...
    <section className={`panel ${styles.scenarioPanel}`} aria-label="시험 설정">
    ...
    {revoked ? <section className={`panel ${styles.revoked}`} role="alert" data-testid="mock-unavailable"><LockKeyhole size={32} /><h2>접근 취소 시험 · 내용 숨김</h2><p>입력·판단 카드·사람 결정·실행 이력의 내용을 표시하지 않습니다.</p><p>실제 OS 권한을 변경한 것은 아닙니다. 시험 사례를 다시 선택하거나 초기화하면 새 시험을 시작할 수 있습니다.</p><button className={`secondary-button ${styles.secondaryButton}`} onClick={() => reset(scenarioId)}>현재 사례 초기화</button></section> : <>
      <div className={styles.workspace}>
        <section className={`panel ${styles.inputPanel}`} aria-label="검토 입력">
          ...
          <button className={`primary-button ${styles.primaryButton}`} onClick={execute}><FlaskConical size={16} /> 모의 검토 실행 <ArrowRight size={16} /></button>
          <p className={styles.micro}>같은 입력의 성공 결과는 재사용합니다. 실패한 시험은 이 버튼을 다시 눌러야 재시도합니다.</p>
          <details className={styles.testControls}><summary>변경·접근 취소 시험</summary><p>화면 안의 시험 상태만 변경합니다. 실제 문서나 권한에는 영향이 없습니다.</p><div><button className={`secondary-button ${styles.secondaryButton}`} onClick={() => changeVersion("source")}>원문 버전 변경</button><button className={`secondary-button ${styles.secondaryButton}`} onClick={() => changeVersion("criteria")}>기준 버전 변경</button><button className={`secondary-button ${styles.secondaryButton}`} onClick={() => { setInput((previous) => ({ ...previous, source: { ...previous.source, access: "denied" } })); setNotice(""); setDecisionError(""); }}>접근 취소 시험</button></div></details>
        </section>
        <section className={`panel ${styles.resultPanel}`} aria-label="검토 결과">
