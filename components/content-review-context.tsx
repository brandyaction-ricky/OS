...
export function ContentReviewContext({ sourceId, sourceVersion, token, disabled }: { sourceId: string; sourceVersion: number; token: string; disabled: boolean }) {
  ...
  return <section className="panel content-workflow-panel" aria-label="집필 전 자료 다시 확인">
    ...
    <p className="content-workflow-note">실시간 감시가 아니며 조회 사이에 발생했다 되돌아온 변경은 감지하지 못할 수 있습니다. 직접 지정하지 않은 공유 자료 전체, 패키징·집필 완료 기준, 대표 승인 증거는 아직 검증하지 않습니다. 자료나 원고는 수정·삭제·생성하지 않습니다.</p>
    </div>
  </section>;
}
