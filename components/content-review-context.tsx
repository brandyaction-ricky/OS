...
export function ContentReviewContext({ sourceId, sourceVersion, token, disabled }: { sourceId: string; sourceVersion: number; token: string; disabled: boolean }) {
  ...
  return <section className="panel" aria-label="집필 전 자료 다시 확인">
    ...
    <p>본인 소유 주제·패키징 자료와 연결된 기획 기준을 조회합니다. 이 화면에서 처음 확인한 자료와 비교하며, 페이지를 다시 열거나 로그인 상태가 바뀌면 비교를 새로 시작합니다.</p>
    ...
    <p>실시간 감시가 아니며 조회 사이에 발생했다 되돌아온 변경은 감지하지 못할 수 있습니다. 다른 소유자의 공유 자료, 패키징·집필 완료 기준, 대표 승인 증거는 아직 검증하지 않습니다. 자료나 원고는 수정·삭제·생성하지 않습니다.</p>
  </section>;
}
