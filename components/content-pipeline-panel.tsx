...
export function ContentPipelinePanel({ sourceId, onChange }: { sourceId: string; onChange: () => Promise<void> }) {
  ...
  return <section className="panel pipeline-panel">
    ...
    {pilotAvailable ? <section className="panel" aria-label="내 목소리 영상 자동화 준비"><h3>내 목소리 + 제작 화면</h3>
      ...
      {currentScenePlan ? <details><summary>현재 원고의 화면 설계 {currentScenePlan.scenes?.length ?? 0}장면</summary><p>{currentScenePlan.visualDirection}</p><ol>{currentScenePlan.scenes?.map((scene) => <li key={scene.segmentIndex}><strong>{scene.segmentIndex + 1}. {scene.layoutTemplate} · {scene.visualType}</strong>{scene.characterAssetRole ? ` · 캐릭터: ${youtubeCharacterAssetRoleLabels[scene.characterAssetRole]}` : ""} · {scene.visualPrompt}{scene.typographyCues?.length ? ` · 음성 타이포: ${scene.typographyCues.map((cue) => `${cue.spokenAnchor} → ${cue.displayText}`).join(" / ")}` : ""}{scene.onScreenText ? ` · 화면 문구: ${scene.onScreenText}` : ""}{scene.evidenceNote ? ` · 근거: ${scene.evidenceNote}` : ""}</li>)}</ol>{currentScenePlan.unresolved?.length ? <p>확인할 항목: {currentScenePlan.unresolved.join(" · ")}</p> : null}</details> : null}
