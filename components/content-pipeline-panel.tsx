...
export function ContentPipelinePanel({ sourceId, onChange }: { sourceId: string; onChange: () => Promise<void> }) {
  ...
  const scenePlan = (state.source.metadata.narratedScenePlan ?? null) as { inputKey?: string; templateVersion?: string; plan?: { visualDirection?: string; scenes?: Array<{ segmentIndex: number; layoutTemplate: string; visualType: string; characterAssetRole?: YoutubeCharacterAssetRole | null; visualBeats?: Array<{ spokenAnchor: string; visualAction: string; composition: string; motionKind: string; motionPacing: string; motionLabels: string[]; motionAccentIndex: number; graphicSpec: string; displayText: string; typographyAnchor: string }>; visualPrompt: string; onScreenText: string; evidenceNote: string }>; unresolved?: string[] } } | null;
  ...
  return <section className="panel pipeline-panel">
    ...
    {pilotAvailable ? <section className="panel" aria-label="내 목소리 영상 자동화 준비"><h3>내 목소리 + 제작 화면</h3>
      ...
      {currentScenePlan ? <details><summary>현재 원고의 화면 설계 {currentScenePlan.scenes?.length ?? 0}장면</summary><p>{currentScenePlan.visualDirection}</p><ol>{currentScenePlan.scenes?.map((scene) => <li key={scene.segmentIndex}><strong>{scene.segmentIndex + 1}. {scene.layoutTemplate} · {scene.visualType}</strong>{scene.characterAssetRole ? ` · 캐릭터: ${youtubeCharacterAssetRoleLabels[scene.characterAssetRole]}` : ""} · {scene.visualPrompt}{scene.visualBeats?.length ? ` · 멘트별 화면: ${scene.visualBeats.map((beat) => `${beat.spokenAnchor} → ${beat.composition} · ${beat.visualAction} · ${beat.motionKind}/${beat.motionPacing}(${beat.motionLabels.join(" → ")}, 강조 ${beat.motionAccentIndex + 1}): ${beat.graphicSpec}${beat.displayText ? ` / ${beat.displayText} (발화 기준: ${beat.typographyAnchor})` : ""}`).join(" / ")}` : ""}{scene.onScreenText ? ` · 화면 문구: ${scene.onScreenText}` : ""}{scene.evidenceNote ? ` · 근거: ${scene.evidenceNote}` : ""}</li>)}</ol>{currentScenePlan.unresolved?.length ? <p>확인할 항목: {currentScenePlan.unresolved.join(" · ")}</p> : null}</details> : null}
