...
import { Archive, ArrowRight, CalendarDays, CheckCircle2, CircleAlert, FileText, Film, Instagram, Layers3, NotebookPen, Plus, Send, Sparkles, X, Youtube } from "lucide-react";
import Link from "next/link";
import { FormEvent, useCallback, useEffect, useMemo, useState } from "react";
import { archiveRecord, createRecord, generateContent, listRecords, updateRecord } from "@/lib/api-client";
...
export function ContentAutomationWorkspace({ initialView = "pipeline" }: { initialView?: AutomationView }) {
  ...
  return <>
    <header className="page-header"><div className="page-title-group"><span className="eyebrow">멀티채널 자동화</span><h1>{view === "review" ? "검토·발행 대기목록" : "멀티채널 자동화"}</h1><p>최종 롱폼 1개를 쇼츠·Threads·SEO 칼럼·카드뉴스·에세이로 전환하고 검토·예약합니다.</p></div><div className="header-actions"><Link className="secondary-button" href="/content/calendar"><CalendarDays size={16} /> 발행 캘린더</Link><button className="secondary-button" onClick={() => setView(view === "pipeline" ? "review" : "pipeline")}><CheckCircle2 size={16} /> {view === "pipeline" ? `대기목록 ${reviewCount}` : "자동화 현황"}</button><button className="primary-button" onClick={() => setSourceOpen(true)}><Plus size={16} /> 롱폼 등록</button></div></header>
