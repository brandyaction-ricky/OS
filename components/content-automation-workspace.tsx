...
export function ContentAutomationWorkspace({ initialView = "pipeline" }: { initialView?: AutomationView }) {
  ...
  const setOutputStatus = async (output: OsRecord, status: string, startsAt?: string, reason?: string) => {
    const rejectionReason = status === "blocked" ? (reason ?? window.prompt("수정 요청 사유를 입력해 주세요.") ?? "").trim() : "";
    if (status === "blocked" && !rejectionReason) return;
    try { await updateRecord(accessToken, { id: output.id, expectedVersion: output.version, status, ...(startsAt ? { startsAt } : {}), metadata: { ...output.metadata, ...(status === "blocked" ? { rejectionReason, rejectedAt: new Date().toISOString() } : {}), ...(status === "review" ? { resubmittedAt: new Date().toISOString() } : {}) } }); await load(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "콘텐츠 상태를 변경하지 못했습니다."); }
  };
  const rejectGroup = async (items: OsRecord[]) => {
    const reason = window.prompt("이 묶음의 수정 요청 사유를 입력해 주세요.")?.trim(); if (!reason) return;
    setBusy(true); setError("");
    try { for (const item of items.filter((entry) => ["draft", "review", "ready"].includes(entry.status))) await updateRecord(accessToken, { id: item.id, expectedVersion: item.version, status: "blocked", metadata: { ...item.metadata, rejectionReason: reason, rejectedAt: new Date().toISOString() } }); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "일부 항목을 변경하지 못했습니다."); }
    finally { await load(); setBusy(false); }
  };
  const completeGroup = async (items: OsRecord[]) => {
    setBusy(true); setError("");
    try {
      for (let item of items.filter((entry) => ["draft", "review"].includes(entry.status))) {
        if (item.status === "draft") item = (await updateRecord(accessToken, { id: item.id, expectedVersion: item.version, status: "review" })).record;
        await updateRecord(accessToken, { id: item.id, expectedVersion: item.version, status: "ready" });
      }
    } catch (reason) { setError(reason instanceof Error ? reason.message : "일부 항목의 검토를 완료하지 못했습니다."); }
    finally { await load(); setBusy(false); }
  };
  ...
  const saveSeoColumn = async (body: string, html: string, imagePrompts: string[]) => { if (!editingOutput) return; setBusy(true); setError(""); try { await updateRecord(accessToken, { id: editingOutput.id, expectedVersion: editingOutput.version, description: body, status: "review", metadata: { ...editingOutput.metadata, derivHtml: html, imagePrompts, columnEditedAt: new Date().toISOString() } }); setEditingOutput(null); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "SEO 칼럼을 저장하지 못했습니다."); } finally { setBusy(false); } };
