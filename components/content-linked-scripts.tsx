...
export function ContentLinkedScripts({ showPlanningHandoff = false }: { showPlanningHandoff?: boolean }) {
  ...
  const [source, setSource] = useState<OsRecord | null>(null);
  ...
  useEffect(() => {
    ...
    let active = true;
    apiRequest<{ source: OsRecord; records: OsRecord[] }>(`/api/v1/content/pipeline?sourceId=${encodeURIComponent(sourceId)}`, { token: accessToken }).then(({ source: loadedSource, records }) => {
      if (!active) return;
      if (loadedSource.id !== sourceId || loadedSource.record_type !== "content_topic") throw new Error("연결된 기획 주제를 확인할 수 없습니다.");
      const linked = records.filter((record) => record.record_type === "content_script").sort((a, b) => b.created_at.localeCompare(a.created_at));
      setSource(loadedSource); setScripts(linked); setDraft(linked[0]?.description ?? "");
