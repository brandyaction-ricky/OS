...
import { AudioLines, Bot, CalendarCheck, CalendarDays, CircleAlert, FileAudio, GitBranch, Mic, Play, Plus, Square, Users, X } from "lucide-react";
import { FormEvent, useCallback, useEffect, useRef, useState } from "react";
import { createRecord, getMeetingRecordingUrl, listRecords, prepareMeeting, summarizeMeeting, transcribeMeeting, updateRecord, uploadMeetingRecording, type MeetingSummaryResult } from "@/lib/api-client";
...
export function MeetingWorkspace() {
  ...
  const [structured, setStructured] = useState<MeetingSummaryResult | null>(null);
  const [prep, setPrep] = useState<Awaited<ReturnType<typeof prepareMeeting>> | null>(null);
  ...
  const openNew = () => { setEditing(null); setSummary(""); setSummaryMode(""); setTranscript(""); setStructured(null); setRecordedBlob(null); setDrawerOpen(true); };
  const openEdit = (meeting: OsRecord) => { setEditing(meeting); setSummary(meta(meeting, "summary")); setSummaryMode(meta(meeting, "summaryMode") as "ai" | "local" | ""); setTranscript(meta(meeting, "transcript")); setStructured({ summary: meta(meeting, "summary"), mode: (meta(meeting, "summaryMode") || "local") as "ai" | "local", decisions: Array.isArray(meeting.metadata.decisions) ? meeting.metadata.decisions.map(String) : [], pending: Array.isArray(meeting.metadata.pending) ? meeting.metadata.pending.map(String) : [], todos: Array.isArray(meeting.metadata.todos) ? meeting.metadata.todos as MeetingSummaryResult["todos"] : [] }); setRecordedBlob(null); setDrawerOpen(true); };
  ...
  const makeSummary = async () => {
    ...
    try { const result = await summarizeMeeting(accessToken, transcript, editing?.starts_at?.slice(0, 10)); setSummary(result.summary); setSummaryMode(result.mode); setStructured(result); }
  ...
  const makeTranscript = async () => {
    ...
    setBusy(true); setError("");
    try { const result = await transcribeMeeting(accessToken, recordedBlob); setTranscript(result.transcript); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "녹음을 전사하지 못했습니다."); }
    finally { setBusy(false); }
  };

  const loadPrep = async () => {
    setBusy(true); setError("");
    try { setPrep(await prepareMeeting(accessToken)); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "회의 준비 자료를 불러오지 못했습니다."); }
    finally { setBusy(false); }
  };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const input = {
        ...
        metadata: { ...(editing?.metadata ?? {}), participants: value("participants"), recordingPath, transcript, summary, summaryMode, decisions: structured?.decisions ?? [], pending: structured?.pending ?? [], todos: structured?.todos ?? [] },
      };
      ...
      for (const title of [...new Set([...(structured?.decisions ?? []), ...decisionLines])]) await createRecord(accessToken, { recordType: "decision", parentId: meeting.id, title, description: `회의: ${meeting.title}`, status: "decided", team: meeting.team, brand: meeting.brand, metadata: { meetingId: meeting.id } });
      const todoRows = [...(structured?.todos ?? []), ...taskLines.map((title) => ({ title, assignee: "", dueDate: "", dueLabel: "" }))];
      for (const todo of todoRows) await createRecord(accessToken, { recordType: "task", parentId: meeting.id, title: todo.title, description: `회의 후속 업무: ${meeting.title}${todo.assignee ? ` · 담당 ${todo.assignee}` : ""}${todo.dueLabel ? ` · ${todo.dueLabel}` : ""}`, status: "planned", team: meeting.team, brand: meeting.brand, dueDate: todo.dueDate || null, metadata: { meetingId: meeting.id, assigneeName: todo.assignee, dueLabel: todo.dueLabel } });
  ...
  return <>
    <header className="page-header"><div className="page-title-group"><span className="eyebrow">MEETING TO ACTION</span><h1>회의·결정</h1><p>지난 미해결 항목과 KPI를 이어받고, 녹음에서 결정과 후속 업무를 만듭니다.</p></div><div className="header-actions"><button className="secondary-button" disabled={busy} onClick={loadPrep}><CalendarCheck size={16} /> 회의 준비</button><button className="primary-button" onClick={openNew}><Plus size={16} /> 회의 기록</button></div></header>
    ...
    {prep ? <section className="panel meeting-prep"><div className="panel-header"><div><h2>다음 회의 준비</h2><p>{prep.latestMeeting ? `이전 회의 “${prep.latestMeeting.title}”에서 이어집니다.` : "첫 회의용 안건입니다."}</p></div><button className="icon-button" onClick={() => setPrep(null)}><X size={16} /></button></div><div className="meeting-prep-grid"><div><strong>미해결 안건</strong>{prep.pending.map((item) => <p key={item}>• {item}</p>)}{!prep.pending.length ? <p>남은 안건이 없습니다.</p> : null}</div><div><strong>완료 전 업무</strong>{prep.todos.slice(0, 8).map((item) => <p key={item.id}>• {item.title}{item.due_date ? ` · ${item.due_date}` : ""}</p>)}{!prep.todos.length ? <p>미완료 업무가 없습니다.</p> : null}</div><div><strong>주간 KPI 안건</strong>{prep.kpis.slice(0, 8).map((item) => <p key={item.id}>• {item.title} {item.current}{item.unit} · {item.signal}</p>)}{!prep.kpis.length ? <p>주간 KPI를 먼저 입력해 주세요.</p> : null}</div></div></section> : null}
