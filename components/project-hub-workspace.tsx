...
function ProjectHubContent() {
  ...
  return <div className="dev-workspace">
    ...
    {current && <details className="dev-project-context"><summary>프로젝트 맥락·협업 방식</summary><p>{current.description}</p><dl><dt>저장소</dt><dd>{meta(current, "repository") || meta(current, "frontendRepository") || "미지정"}</dd><dt>운영 주소</dt><dd>{meta(current, "productionUrl") || "미지정"}</dd><dt>배포 규칙</dt><dd>{meta(current, "deploymentRule") || "프로젝트 저장소 지침 확인"}</dd><dt>협업</dt><dd>요청 접수 → Work 인수인계 → 브랜치·PR → 검증 → 승인된 배포 → 같은 요청에 결과 기록</dd></dl></details>}
