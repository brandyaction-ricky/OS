...
import { createDevelopmentAttachmentUpload, createRecord, deleteDevelopmentAttachment, getDevelopmentAttachmentUrl, listMembers, listRecords, updateRecord, uploadDevelopmentAttachment, type OsMember } from "@/lib/api-client";
...
import { DevelopmentProjectOverview } from "./development-project-overview";
...
const PAGE_SIZE = 25;
type Modal = "project" | "projectEdit" | "request" | "featureRequest" | "edit" | "log" | "deployment" | null;
...
function developmentDocumentId(value: string) {
  const trimmed = value.trim();
  if (!trimmed) return "";
  if (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(trimmed)) return trimmed;
  try { return new URL(trimmed, "https://brandyaction-os.vercel.app").searchParams.get("document") || ""; } catch { return ""; }
}
function developmentDocumentUrl(id: string) { return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id) ? `/knowledge?document=${encodeURIComponent(id)}` : ""; }
...
function ProjectHubContent() {
  ...
  const [tab, setTab] = useState<"requests" | "history" | "context" | "connections" | "guide">("requests");
  ...
  useEffect(() => {
    ...
    setStatus(FLOW.includes(params.get("status") || "") ? params.get("status") || "" : "");
    setScope(params.get("scope") === "mine" ? "mine" : "");
    setSelected(null);
  ...
  useEffect(() => {
    const requestedProject = initialProject.current;
    if (!requestedProject || !projects.some(item => item.id === requestedProject)) return;
    setProjectId(requestedProject);
    initialProject.current = "";
  }, [navigationQuery, projects]);
  ...
  useEffect(() => {
    ...
    if (demo) {
      ...
      setProjects([project, previewRecord({ id: "preview-edu", record_type: "project", title: "브랜디에듀", description: "강의 판매·수강·교육 운영", status: "active" }), previewRecord({ id: "preview-myin", record_type: "project", title: "마이인", description: "진단·보고서·회원 경험", status: "active" })]); setLoading(false);
      return;
    }
    projectRecords(accessToken, "project").then(records => {
      if (cancelled) return;
      setProjects([...records].sort((a, b) => (["OS", "Edu", "Myin"].indexOf(projectLabel(a.title)) + 4) % 4 - (["OS", "Edu", "Myin"].indexOf(projectLabel(b.title)) + 4) % 4));
      setProjectId(previous => previous || records.find(item => item.id === initialProject.current)?.id || "");
      setLoading(false);
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [accessToken, demo, navigationQuery]);

  useEffect(() => { setOffset(0); }, [projectId, status, query, scope]);
  useEffect(() => {
    if ((!projectId && !status && !scope) || sessionLoading || (!demo && !accessToken)) return;
    let cancelled = false;
    let pending = false;
    const load = async (quiet = false) => {
      ...
      try {
        let result: Inbox;
        if (demo) {
          const all = demoRequests.filter(item => (!projectId || item.parent_id === projectId) && (!query || `${item.title} ${item.description}`.includes(query)) && (!scope || item.created_by === profile?.id));
        ...
        } else result = await requestApi<Inbox>(accessToken, new URLSearchParams({ ...(projectId ? { projectId } : {}), ...(status ? { status } : {}), ...(scope ? { scope } : {}), q: query, limit: String(PAGE_SIZE), offset: String(offset) }).toString());
  ...
  const openAttachment = async (request: OsRecord) => {
    const path = meta(request, "attachmentPath");
    if (!path) return;
    try {
      const { url } = await getDevelopmentAttachmentUrl(accessToken, path);
      const anchor = document.createElement("a"); anchor.href = url; anchor.target = "_blank"; anchor.rel = "noreferrer"; anchor.click();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "첨부 자료를 열지 못했습니다."); }
  };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (modal === "request" || modal === "featureRequest" || modal === "edit") {
        const file = form.get("attachment");
        let attachment = modal === "edit" && selected ? { path: meta(selected, "attachmentPath"), name: meta(selected, "attachmentName"), size: meta(selected, "attachmentSize"), type: meta(selected, "attachmentType") } : { path: "", name: "", size: "", type: "" };
        if (!demo && file instanceof File && file.size > 0) {
          const signed = await createDevelopmentAttachmentUpload(accessToken, file);
          uploadedPath = signed.path;
          await uploadDevelopmentAttachment(signed.path, signed.token, file, signed.type);
          attachment = { path: signed.path, name: signed.name, size: String(signed.size), type: signed.type };
        } else if (demo && file instanceof File && file.size > 0) attachment = { path: "", name: file.name, size: String(file.size), type: file.type };
        const body = { title: value("title"), description: value("description"), parentId: value("parentId"), priority: value("priority"), category: value("category"), pageUrl: value("pageUrl"), expectedResult: value("expectedResult"), attachmentUrl: modal === "edit" && selected ? meta(selected, "attachmentUrl") : "", attachmentPath: attachment.path, attachmentName: attachment.name, attachmentSize: attachment.size, attachmentType: attachment.type, ...(modal === "edit" && selected ? { id: selected.id, expectedVersion: selected.version } : {}) };
        let saved: OsRecord;
        if (demo) {
          saved = previewRecord({ ...selected && modal === "edit" ? selected : {}, title: body.title, description: body.description, parent_id: body.parentId, priority: body.priority as OsRecord["priority"], metadata: { ...(modal === "edit" ? selected?.metadata : {}), kind: "development_request", pageUrl: body.pageUrl, category: body.category, expectedResult: body.expectedResult, attachmentUrl: body.attachmentUrl, attachmentPath: body.attachmentPath, attachmentName: body.attachmentName, attachmentSize: body.attachmentSize, attachmentType: body.attachmentType }, version: modal === "edit" ? (selected?.version || 0) + 1 : 1 });
        ...
        const previousPath = modal === "edit" && selected ? meta(selected, "attachmentPath") : "";
        if (!demo && uploadedPath && previousPath && previousPath !== uploadedPath) void deleteDevelopmentAttachment(accessToken, previousPath).catch(() => undefined);
        setSelected(saved); setProjectId(saved.parent_id || projectId); setStatus(""); setQuery(""); setSearch(""); setOffset(0); setTab("requests");
        setNotice(modal === "edit" ? "요청 내용을 수정했습니다." : modal === "featureRequest" ? "추가 개발 요청이 접수됐습니다." : "수정 요청이 접수됐습니다.");
      } else if (modal === "project" || modal === "projectEdit") {
        const repository = value("repository");
        if (repository && !repositoryUrl(repository)) throw new Error("저장소는 owner/repository 또는 GitHub 저장소 주소로 입력해 주세요.");
        const documentId = developmentDocumentId(value("developmentDocument"));
        if (documentId && !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(documentId)) throw new Error("정본 문서 주소 또는 문서 ID를 확인해 주세요.");
        const metadata = { ...(modal === "projectEdit" ? current?.metadata : {}), repository, productionUrl: value("productionUrl"), developmentDocumentId: documentId };
        const body = { recordType: "project", title: value("title"), description: value("description"), status: "active", metadata };
        const saved = demo
          ? previewRecord({ ...(modal === "projectEdit" ? current || {} : {}), record_type: "project", title: body.title, description: body.description, status: "active", metadata, version: modal === "projectEdit" ? (current?.version || 0) + 1 : 1 })
          : modal === "projectEdit" && current
            ? (await updateRecord(accessToken, { ...body, id: current.id, expectedVersion: current.version })).record
            : (await createRecord(accessToken, body)).record;
        setProjects(previous => modal === "projectEdit" ? previous.map(item => item.id === saved.id ? saved : item) : [...previous, saved]); setProjectId(saved.id); if (!demo) setProjectRevision(value => value + 1); setNotice(modal === "projectEdit" ? "프로젝트 연결 정보를 저장했습니다." : "프로젝트를 등록했습니다.");
  ...
  const deleteSelected = async () => {
    if (!selected || saving || !window.confirm(`“${selected.title}” 요청을 삭제할까요?\n삭제한 요청은 목록에서 사라집니다.`)) return;
    setSaving(true); setError("");
    try {
      if (demo) setDemoRequests(previous => previous.filter(item => item.id !== selected.id));
      else await requestApi<{ archived: true }>(accessToken, new URLSearchParams({ id: selected.id, expectedVersion: String(selected.version) }).toString(), undefined, "DELETE");
      setSelected(null); setOffset(0); changed(); setNotice("개발 요청을 삭제했습니다.");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "개발 요청을 삭제하지 못했습니다."); }
    finally { setSaving(false); }
  };
  ...
  return <div className="dev-workspace">
    ...
    <nav className="dev-project-navigation" aria-label="프로젝트 전환"><button disabled={saving} aria-current={!projectId ? "page" : undefined} onClick={() => chooseProject("")}>전체 현황</button>{projects.map(item => <button key={item.id} disabled={saving} aria-current={projectId === item.id ? "page" : undefined} onClick={() => chooseProject(item.id)}>{projectLabel(item.title)}</button>)}</nav>
    <div className="dev-project-bar"><label><FolderGit2 size={15} /><select aria-label="개발 프로젝트 선택" disabled={saving} value={projectId} onChange={event => { setProjectId(event.target.value); setSelected(null); }}><option value="">전체 현황</option>{projects.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><SafeLink url={current ? repositoryUrl(meta(current, "repository")) : null}>GitHub 저장소</SafeLink>{current && developmentDocumentUrl(meta(current, "developmentDocumentId")) ? <a href={developmentDocumentUrl(meta(current, "developmentDocumentId"))}>프로젝트 정본<ArrowUpRight size={13} /></a> : null}{admin && <button className="dev-text-button" onClick={() => open("project")}><Plus size={13} /> 프로젝트 추가</button>}</div>
    {!current && !status && !scope && <DevelopmentProjectOverview projects={projects} token={accessToken} demo={demo} requests={demoRequests} onChoose={chooseProject} />}
    {current && <div className="dev-tabs" role="tablist" aria-label="개발 관리 보기">{([["requests", "개발 요청"], ["history", "개발·배포 기록"], ["context", "프로젝트 맥락"], ["connections", "연결 정보"], ["guide", "운영 방법"]] as const).map(([id, label]) => <button key={id} role="tab" aria-selected={tab === id} onClick={() => setTab(id)}>{label}{id === "requests" && <span>{loading ? "…" : pending}</span>}</button>)}</div>}
    {current && tab === "context" && <details open className="dev-project-context"><summary>프로젝트 맥락·협업 방식</summary><p>{current.description}</p><dl><dt>저장소</dt><dd>{meta(current, "repository") || meta(current, "frontendRepository") || "미지정"}</dd><dt>운영 주소</dt><dd>{meta(current, "productionUrl") || "미지정"}</dd><dt>배포 규칙</dt><dd>{meta(current, "deploymentRule") || "프로젝트 저장소 지침 확인"}</dd><dt>현재 우선순위</dt><dd>{meta(current, "currentPriority") || "최근 개발 기록과 미완료 요청을 확인하세요."}</dd><dt>협업</dt><dd>요청 접수 → Work 인수인계 → 브랜치·PR → 검증 → 승인된 배포 → 같은 요청에 결과 기록</dd></dl></details>}
    {(current || status || scope) && tab === "requests" && <>
    ...
    {current && tab === "history" && <section className="dev-history"><header><div><h2>변경과 배포의 기록</h2><p>작업 기록과 실제 배포 결과를 각각 남깁니다.</p></div>{admin && <div className="dev-actions"><button className="dev-button" disabled={!current} onClick={() => open("log")}><FileText size={14} /> 개발 기록</button><button className="dev-button" disabled={!current} onClick={() => open("deployment")}><Rocket size={14} /> 배포 기록</button></div>}</header>{historyError && <div className="dev-error" role="alert">{historyError}<button onClick={retryAll}>다시 불러오기</button></div>}{historyLoading ? <div className="dev-empty"><Loader2 className="dev-spin" size={22} />기록을 불러오는 중입니다.</div> : history.filter(item => item.parent_id === projectId).map(item => <article key={item.id}><span className="dev-history-icon">{item.record_type === "deployment" ? <Rocket size={16} /> : <GitCommitHorizontal size={16} />}</span><div><div className="dev-history-title"><h3>{item.title}</h3><Status value={item.status} /></div><p>{item.description}</p><small>{date(item.created_at)} · {item.stage || "환경 미기재"} {meta(item, "branch") && `· ${meta(item, "branch")}`} {meta(item, "commitSha") && `· ${meta(item, "commitSha").slice(0, 12)}`}</small>{meta(item, "requestId") && <a className="dev-history-request-link" href={`/knowledge/development?request=${encodeURIComponent(meta(item, "requestId"))}`}>연결 요청 · {meta(item, "requestId").slice(0, 8)}<ArrowUpRight size={13} /></a>}{Array.isArray(item.metadata.checks) && <ul>{item.metadata.checks.filter(check => typeof check === "string").map((check, index) => <li key={index}>{String(check)}</li>)}</ul>}{Array.isArray(item.metadata.nextSteps) && item.metadata.nextSteps.length > 0 && <p className="dev-next-steps">다음 작업: {item.metadata.nextSteps.map(String).join(" · ")}</p>}<SafeLink url={item.source_url}>결과 보기</SafeLink></div></article>)}{!historyLoading && !historyError && !history.some(item => item.parent_id === projectId) && <div className="dev-empty"><GitCommitHorizontal size={25} /><h3>아직 작업 기록이 없습니다</h3><p>Work에서 수정한 뒤 브랜치·커밋·검증 결과를 남기세요.</p></div>}</section>}
    {current && tab === "guide" && <section className="dev-guide"><div><span className="dev-kicker">팀과 함께</span><h2>불편함과 아이디어를 바로 요청하세요.</h2><p>불편한 점은 ‘수정 요청’, 새 기능 아이디어는 ‘추가 개발 요청’으로 남기세요. 자료나 이미지를 파일로 함께 올릴 수 있습니다.</p><ol><li><strong>직원 · 요청 남기기</strong><span>프로젝트·요청 내용·기대 결과와 참고 자료를 기록합니다.</span></li><li><strong>리키 · Work에서 수정하기</strong><span>요청 상세의 ‘Work에 전달’을 복사해 새 채팅에서 작업을 시작합니다.</span></li><li><strong>개발 · GitHub에 버전 남기기</strong><span>작업 브랜치와 PR로 수정 내용을 저장하고 테스트·Preview를 확인합니다.</span></li><li><strong>운영 · 검수하고 결과 남기기</strong><span>같은 요청에 처리 결과와 반영 화면을 연결합니다. 문제가 남으면 다시 요청할 수 있습니다.</span></li></ol></div><aside><h3>세 곳의 역할</h3><p><strong>회사 OS</strong> 요청·처리 상태·개발·배포 기록</p><p><strong>ChatGPT Work</strong> 분석·코드 수정·검증</p><p><strong>GitHub</strong> 코드 버전·변경 검토·되돌리기</p><hr /><p>요청은 OS에 접수되며 열린 화면은 60초마다, 탭에 돌아오면 즉시 갱신됩니다.</p><p>Work 작업은 요청의 맥락을 전달해 시작합니다. 요청 등록만으로 자동 수정·배포가 실행되지는 않습니다.</p><button className="dev-button dev-wide" disabled={!current} onClick={() => copy()}><Copy size={14} /> 프로젝트 이어가기 복사</button></aside></section>}
    {current && tab === "connections" && <section className="dev-guide"><div><h2>{current.title} 연결 정보</h2><p>개발 환경에서 검증한 뒤 운영 반영 상태를 별도로 기록합니다.</p>{[["GitHub 저장소", repositoryUrl(meta(current, "repository") || meta(current, "frontendRepository"))], ["백엔드 저장소", repositoryUrl(meta(current, "backendRepository"))], ["개발 사이트", meta(current, "developmentUrl") || meta(current, "devUrl")], ["운영 사이트", meta(current, "productionUrl")]].map(([label, url]) => <p key={label}>{label}: {safeWebUrl(url) ? <SafeLink url={url}>{url}</SafeLink> : "미등록"}</p>)}<p>프로젝트 정본: {developmentDocumentUrl(meta(current, "developmentDocumentId")) ? <a href={developmentDocumentUrl(meta(current, "developmentDocumentId"))}>{developmentDocumentUrl(meta(current, "developmentDocumentId"))}<ArrowUpRight size={13} /></a> : "미등록"}</p><p>개발 브랜치: {meta(current, "developmentBranch") || meta(current, "devBranch") || "미등록"}</p><p>운영 브랜치: {meta(current, "productionBranch") || "미등록"}</p>{admin ? <button className="dev-button" onClick={() => open("projectEdit")}>연결 정보 수정</button> : null}</div><aside><h3>정본과 개발 기록</h3><p>프로젝트 정본 문서를 연결하면 해당 문서의 읽기 화면에 요청 상태와 개발·배포 기록이 실시간으로 표시됩니다.</p><p>자유 서술 본문과 버전은 자동으로 수정하지 않습니다.</p></aside></section>}
      {(modal || copyFallback) && <div className="dev-modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget && !saving) { setModal(null); setCopyFallback(""); } }}><div ref={modalRef} className="dev-modal" role="dialog" aria-modal="true" aria-labelledby="dev-modal-title"><header><div><span className="dev-kicker">{copyFallback ? "Work 인수인계" : "개발 관리"}</span><h2 id="dev-modal-title">{copyFallback ? "아래 내용을 복사해 주세요" : modal === "project" ? "프로젝트 추가" : modal === "projectEdit" ? "프로젝트 연결 수정" : modal === "log" ? "개발 기록 남기기" : modal === "deployment" ? "배포 결과 남기기" : modal === "edit" ? "요청 내용 수정" : modal === "featureRequest" ? "어떤 기능이 더 필요하신가요?" : "무엇이 불편하셨나요?"}</h2></div><button disabled={saving} className="dev-icon-button" aria-label="입력창 닫기" onClick={() => { setModal(null); setCopyFallback(""); }}><X size={19} /></button></header>{copyFallback ? <div className="dev-modal-fields"><textarea aria-label="Work 전달 내용" readOnly rows={20} value={copyFallback} onFocus={event => event.target.select()} /></div> : <form onSubmit={submit}><div className="dev-modal-fields">{formError && <div className="dev-error" role="alert">{formError}</div>}
      {(modal === "request" || modal === "featureRequest" || modal === "edit") ? <><label>프로젝트<select name="parentId" required defaultValue={modal === "edit" ? selected?.parent_id || projectId : projectId}><option value="" disabled>프로젝트를 선택하세요</option>{projects.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><div className="dev-form-grid"><label>요청 종류<select name="category" defaultValue={modal === "edit" ? meta(selected, "category") || "bug" : modal === "featureRequest" ? "feature" : "bug"}>{Object.entries(CATEGORIES).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>업무 영향<select name="priority" defaultValue={modal === "edit" ? selected?.priority : "normal"}><option value="normal">보통 · 사용은 가능해요</option><option value="high">높음 · 업무가 지연돼요</option><option value="urgent">긴급 · 업무를 못해요</option><option value="low">낮음 · 나중에 개선해도 돼요</option></select></label></div><label>제목<input name="title" required maxLength={240} defaultValue={modal === "edit" ? selected?.title : ""} placeholder={modal === "featureRequest" ? "예: 요청에 담당자와 완료 예정일을 지정하고 싶어요" : "예: 원고를 저장해도 목록에 나타나지 않아요"} /></label><label>{modal === "featureRequest" ? "관련 페이지" : "문제가 발생한 페이지"}<input name="pageUrl" type="url" maxLength={2000} defaultValue={modal === "edit" ? fullPageUrl(meta(selected, "pageUrl")) : pageUrl} placeholder="https://brandyaction-os.vercel.app/…" /></label><label>{modal === "featureRequest" ? "어떤 기능이 필요한가요?" : "현재 어떤 문제가 있나요?"}<textarea name="description" required rows={4} maxLength={10000} defaultValue={modal === "edit" ? selected?.description : ""} placeholder={modal === "featureRequest" ? "누가, 언제, 어떤 목적으로 사용할 기능인지 적어 주세요." : "누른 버튼과 화면에 나온 결과를 적어 주세요."} /></label><label>{modal === "featureRequest" ? "어떻게 사용되면 좋을까요?" : "어떤 결과를 원하시나요?"}<textarea name="expectedResult" required rows={3} maxLength={5000} defaultValue={modal === "edit" ? meta(selected, "expectedResult") : ""} placeholder="원하는 화면이나 동작을 적어 주세요." /></label><label className="dev-file-field"><span>자료 또는 이미지 <small>선택 · 최대 25MB</small></span><span className="dev-file-picker"><UploadCloud size={18} /><span>{modal === "edit" && meta(selected, "attachmentName") ? "현재 파일: " + meta(selected, "attachmentName") : "이미지·영상·문서 파일 선택"}</span><input name="attachment" type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/quicktime,video/webm,application/pdf,text/plain,text/csv,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.zip" /></span></label><p className="dev-field-help">요청 내용과 첨부 자료는 회사 구성원이 확인합니다. 비밀번호나 고객 개인정보는 입력하지 마세요.</p></> : modal === "project" || modal === "projectEdit" ? <><label>프로젝트명<input name="title" required maxLength={240} defaultValue={modal === "projectEdit" ? current?.title : ""} placeholder="프로젝트명" /></label><label>목적과 완료 기준<textarea name="description" required rows={4} defaultValue={modal === "projectEdit" ? current?.description : ""} placeholder="이 프로젝트로 해결할 업무와 완료 기준" /></label><label>GitHub 저장소<input name="repository" defaultValue={modal === "projectEdit" ? meta(current, "repository") : ""} placeholder="owner/repository" /></label><label>운영 주소<input name="productionUrl" type="url" defaultValue={modal === "projectEdit" ? meta(current, "productionUrl") : ""} placeholder="https://…" /></label><label>프로젝트 정본 문서<input name="developmentDocument" defaultValue={modal === "projectEdit" ? developmentDocumentUrl(meta(current, "developmentDocumentId")) : ""} placeholder="OS 문서 주소 또는 문서 ID" /></label><p className="dev-field-help">연결된 정본을 열면 요청·상태·개발·배포 기록이 실시간으로 표시됩니다. 문서 본문은 자동 수정하지 않습니다.</p></> : <><label>기록 제목<input name="title" required maxLength={240} placeholder="예: 수정요청 접수·처리 기능 반영" /></label><label>변경 내용과 결과<textarea name="description" required maxLength={10000} rows={5} /></label><div className="dev-form-grid"><label>환경<select name="environment" defaultValue="local"><option value="local">로컬</option><option value="dev">Preview</option><option value="production">운영</option></select></label><label>상태<select name="status" defaultValue={modal === "log" ? "working" : "ready"}>{(modal === "log" ? ["working", "tested", "dev_deployed", "review", "completed", "blocked"] : ["deploying", "ready", "failed", "rolled_back"]).map(value => <option key={value} value={value}>{LABELS[value]}</option>)}</select></label></div><label>브랜치<input name="branch" maxLength={200} placeholder="codex/수정주제-날짜" /></label><label>커밋 SHA<input name="commitSha" required={modal === "deployment"} pattern="[a-fA-F0-9]{7,40}" /></label><label>검증 내용 <small>한 줄에 하나씩</small><textarea name="checks" required rows={4} placeholder="테스트 통과\n원고 저장 후 새로고침하여 유지 확인\n모바일 확인 미실시" /></label><label>결과 또는 배포 주소<input name="resultUrl" type="url" required={modal === "deployment"} /></label><label>남은 문제와 다음 작업<textarea name="nextSteps" rows={3} /></label><p className="dev-field-help">이 양식은 이미 수행한 작업을 기록합니다. 저장으로 코드 배포가 실행되지는 않습니다.</p></>}
      </div><footer><button type="button" className="dev-button" disabled={saving} onClick={() => setModal(null)}>취소</button><button className="dev-button primary" disabled={saving || ((modal === "request" || modal === "featureRequest" || modal === "edit") && !projects.length)}>{saving ? <><Loader2 className="dev-spin" size={14} /> 업로드·저장 중…</> : <><Check size={14} />{modal === "request" ? "수정 요청 등록" : modal === "featureRequest" ? "추가 개발 요청 등록" : "저장"}</>}</button></footer></form>}</div></div>}
  </div>;
}
