...
function developmentDocumentId(value: string) {
  ...
  if (!trimmed) return "";
  try { return new URL(trimmed).searchParams.get("document") || ""; } catch { return trimmed; }
}
function developmentDocumentUrl(id: string) { return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id) ? `https://brandyaction-os.vercel.app/knowledge?document=${encodeURIComponent(id)}` : ""; }
...
function ProjectHubContent() {
  ...
  return <div className="dev-workspace">
    ...
    {current && tab === "connections" && <section className="dev-guide"><div><h2>{current.title} 연결 정보</h2><p>개발 환경에서 검증한 뒤 운영 반영 상태를 별도로 기록합니다.</p>{[["GitHub 저장소", repositoryUrl(meta(current, "repository") || meta(current, "frontendRepository"))], ["백엔드 저장소", repositoryUrl(meta(current, "backendRepository"))], ["개발 사이트", meta(current, "developmentUrl") || meta(current, "devUrl")], ["운영 사이트", meta(current, "productionUrl")], ["프로젝트 정본", developmentDocumentUrl(meta(current, "developmentDocumentId"))]].map(([label, url]) => <p key={label}>{label}: {safeWebUrl(url) ? <SafeLink url={url}>{url}</SafeLink> : "미등록"}</p>)}<p>개발 브랜치: {meta(current, "developmentBranch") || meta(current, "devBranch") || "미등록"}</p><p>운영 브랜치: {meta(current, "productionBranch") || "미등록"}</p>{admin ? <button className="dev-button" onClick={() => open("projectEdit")}>연결 정보 수정</button> : null}</div><aside><h3>정본과 개발 기록</h3><p>프로젝트 정본 문서를 연결하면 해당 문서의 읽기 화면에 요청 상태와 개발·배포 기록이 실시간으로 표시됩니다.</p><p>자유 서술 본문과 버전은 자동으로 수정하지 않습니다.</p></aside></section>}
