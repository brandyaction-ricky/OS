...
function ProjectHubContent() {
  ...
  return <div className="dev-workspace">
    ...
    {current && <div className="dev-tabs" role="tablist" aria-label="개발 관리 보기">{([["requests", "수정요청"], ["history", "개발·배포 기록"], ["context", "프로젝트 맥락"], ["connections", "연결 정보"], ["guide", "운영 방법"]] as const).map(([id, label]) => <button key={id} role="tab" aria-selected={tab === id} onClick={() => setTab(id)}>{label}{id === "requests" && <span>{loading ? "…" : pending}</span>}</button>)}</div>}
