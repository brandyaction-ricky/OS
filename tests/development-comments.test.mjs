...
test("development comment input trims content and validates request relationships", () => {
  const mentionId = "7ed5bf4d-728b-4ee1-b90e-b6e91cbd2629";
  const parsed = developmentCommentCreateSchema.parse({ requestId, body: "  확인했습니다.  ", replyTo, mentionIds: [mentionId] });
  assert.equal(parsed.body, "확인했습니다.");
  assert.equal(parsed.replyTo, replyTo);
  assert.deepEqual(parsed.mentionIds, [mentionId]);
  ...
  assert.equal(developmentCommentCreateSchema.safeParse({ requestId, body: "내용", mentionIds: [mentionId, mentionId] }).success, false);
});
...
test("development comment metadata keeps an immutable request and reply link", () => {
  assert.deepEqual(developmentCommentMetadata({ requestId, replyTo, authorName: "브랜디", authorType: "member" }), {
    kind: "development_comment", requestId, replyTo, authorName: "브랜디", authorType: "member", mentionIds: [], mentionNames: [],
  });
