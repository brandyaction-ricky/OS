...
test("route and UI keep the experiment read-only and server-side", async () => {
  ...
  assert.match(route, /evaluationProtocolVersion: "jev-packaging-review-v3"/);
  assert.match(route, /factualVerification: false/);
  assert.match(ui, /사람의 직감 점수를 정답으로 입력하지 않습니다/);
  assert.match(ui, /JEV 평가 실행/); assert.match(ui, /외부 출처 사실 확인 미실시/);
  assert.match(ui, /클릭률·실제 성과·정확도 또는 발행 승인으로 해석하지 마세요/);
  assert.match(ui, /전략적 채택·수정·보류는 기존 기획 절차에서 결정/);
  assert.doesNotMatch(ui, /humanScores|humanRisk|humanReason|Math\.abs\(score\.score - human\)/);
  assert.match(ui, /저장·승인·단계 이동에 사용하지 않습니다/);
