...
test("route and UI keep the experiment read-only and server-side", async () => {
  ...
  assert.match(ui, /사람 판정 5개와 판정 이유를 먼저 입력/); assert.match(ui, /사람 판정 · 이유/);
  assert.match(ui, /humanReason\.trim\(\)\.length < 10/); assert.match(ui, /평가 데이터로 저장되지 않습니다/);
  assert.match(ui, /승인·저장·단계 이동에는 사용하지 않습니다/);
