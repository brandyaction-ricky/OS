...
export const JEV_CONTENT_ASSIST_CONTRACT = "jev-content-packaging-advisory-v1";
...
export const JEV_CONTENT_ASSIST_QUESTIONS = Object.freeze({
  topic_relevance: { type: "score", instructions: "이 주제가 타깃 시청자의 현재 일과 의사결정에 얼마나 직접적으로 관련되는지 평가한다.", criteria: ["관련성이 거의 없다", "간접적인 관련만 있다", "관련은 있지만 긴급하거나 구체적이지 않다", "직접적이고 구체적인 관련이 있다", "매우 직접적이며 지금 확인할 이유가 분명하다"] },
  thumbnail_clarity: { type: "score", instructions: "제목을 보지 않고 썸네일 카피만 읽었을 때 영상이 다룰 핵심 문제가 얼마나 분명한지 평가한다.", criteria: ["내용을 추론하기 어렵다", "분야만 짐작할 수 있다", "핵심 문제를 대략 짐작할 수 있다", "핵심 문제가 분명하다", "핵심 문제와 시청 이유가 모두 즉시 분명하다"] },
  curiosity_strength: { type: "score", instructions: "제목과 썸네일 조합이 답을 확인하고 싶게 만드는 정보 격차를 얼마나 강하게 만드는지 평가한다.", criteria: ["궁금증이 거의 없다", "약한 궁금증만 생긴다", "일정한 궁금증이 생긴다", "강한 궁금증이 생긴다", "즉시 클릭하고 싶을 정도로 강하다"] },
  evidence_boundary: { type: "score", instructions: "입력된 설명과 근거 메모가 사실·전망·해석의 경계를 얼마나 명확하게 구분하는지 평가한다.", criteria: ["구분이 없어 오해 위험이 매우 크다", "구분이 불명확하다", "일부 구분하지만 혼동 가능성이 있다", "대체로 명확히 구분한다", "명시적이고 일관되게 구분한다"] },
  overclaim_risk: { type: "choice", instructions: "제목과 썸네일이 입력된 설명·근거가 허용하는 범위를 넘어 단정하거나 과장할 위험 수준을 선택한다.", criteria: {
...
export function buildJevContentAssistState(input: unknown) {
  const material = materialSchema.parse(input);
  return ["콘텐츠 형식: 유튜브 콘텐츠", `타깃: ${material.audience}`, `제목: ${material.title}`,
    `썸네일 카피: ${material.thumbnailCopy}`, `핵심 내용: ${material.coreContent}`,
    `근거 메모: ${material.evidenceNotes}`, `시청자에게 주는 약속: ${material.viewerPromise}`].join("\n");
}

type Fetcher = (input: string, init: RequestInit) => Promise<Response>;
export async function evaluateJevContentAssist(input: unknown, options: { apiKey: string; model?: string; fetcher?: Fetcher; signal?: AbortSignal }) {
  const apiKey = options.apiKey.trim();
  if (!apiKey) throw new Error("JEV_NOT_CONFIGURED");
  const state = buildJevContentAssistState(input);
  const startedAt = Date.now();
  const response = await (options.fetcher ?? fetch)(JEV_SYSTEM_ONE_ENDPOINT, {
    method: "POST", signal: options.signal,
    headers: { authorization: `Bearer ${apiKey}`, "content-type": "application/json", accept: "application/json" },
    body: JSON.stringify({ state, model: options.model?.trim() || "jev-latest", questions: JEV_CONTENT_ASSIST_QUESTIONS }),
  });
  ...
  return Object.freeze({
    contractVersion: JEV_CONTENT_ASSIST_CONTRACT,
