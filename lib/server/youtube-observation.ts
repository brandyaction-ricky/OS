...
import { outlierBaseline, youtubeAgeCheckpoint, youtubeFormatBucket, type BaselineVideo } from "@/lib/youtube-outliers";
...
export async function observeYoutubeChannels(now = new Date()) {
  ...
  const counts = { channels: channels.length, videos: 0, metricsCreated: 0, metricsUpdated: 0, outliersCreated: 0, failures: 0 };
  for (const channel of channels) {
    try {
      const videos = await channelVideos(channel);
      counts.videos += videos.length;
      const observationKeys = videos.map((video) => `${channel.channelId}:${video.id}:${day}`);
      ...
      for (const video of videos) {
        const observationKey = `${channel.channelId}:${video.id}:${day}`;
        const metadata = {
          observationKey, contentId: video.id, channelId: channel.channelId, channelTitle: video.channelTitle || channel.title,
          platform: "YouTube", dataSource: "youtube_data_api", metricMode: "cumulative", measuredAt: now.toISOString(),
          views: video.views, likes: video.likes, comments: video.comments, durationSeconds: video.durationSeconds,
          formatBucket: youtubeFormatBucket(video.durationSeconds), ageCheckpoint: youtubeAgeCheckpoint(video.publishedAt, now.getTime()), publishedAt: video.publishedAt,
        };
        const payload = { title: video.title, description: "추적 채널 일일 자동 측정", status: "measuring", priority: "normal", stage: "채널 자동 수집", team: "콘텐츠", brand: "브랜디액션", owner_id: channel.ownerId, updated_by: channel.ownerId, starts_at: `${day}T12:00:00.000Z`, metric_current: video.views, metric_unit: "조회", source_url: `https://www.youtube.com/watch?v=${video.id}`, tags: ["YouTube", "자동측정", youtubeAgeCheckpoint(video.publishedAt, now.getTime())].filter(Boolean), metadata };
