...
[틀 고르기]
글자보다 도식을 먼저 고릅니다. 멘트의 논리 구조가 틀과 실제로 맞을 때 그 도식을 씁니다: 차례·인과는 steps, 반복·악순환은 cycle, 무게·비중 비교는 balance, 많은 것이 걸러짐은 funnel, 섞임·겹침은 venn, 순서·우선순위는 ranking, 흔한 생각과 진짜 답은 compare, 원인 정리는 formula, 감정·상황은 character_labels(캐릭터 목록에서 뜻이 맞는 id), 조언 나열은 list입니다.
억지 도식은 금지입니다. 구조가 맞지 않으면 도식 대신 character_labels나 글자 틀을 씁니다. 글자 틀(statement, question)은 이야기 덩어리의 시작과 핵심 결론에만 쓰고, 전체 비트의 3분의 1을 넘지 않게 합니다.
...
같은 틀을 연달아 두 번 쓰지 않습니다. character_labels를 영상 곳곳에 섞되 연달아 남발하지 않습니다.
