...
export function outlierBaseline(target: BaselineVideo, candidates: BaselineVideo[], now = Date.now()) {
  ...
  const sample = candidates.filter((video) => video.id !== target.id && video.channelId === target.channelId && !excluded(video) && age(video) >= targetAge * .5 && age(video) <= targetAge * 2)
  ...
  return { state: "measured", sampleCount: sample.length, ratio, robustZ, medianViews: baseline, outlier: !news && ((ratio !== null && ratio >= 3) || (robustZ !== null && robustZ >= 3.5)), reason: news ? "뉴스 카테고리 · 화제성 별도 검토" : "같은 채널 롱폼 20개 · 경과일 0.5~2배 범위", sampleIds: sample.map((video) => video.id) };
}
