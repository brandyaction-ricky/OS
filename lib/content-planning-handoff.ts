...
const formatChangeSchema = z.object({ from: productionFormatSchema, to: productionFormatSchema, sourceVersion: z.number().int().positive() }).strict().refine(value => value.from !== value.to);

// Working context only; this is not a decision, approval, or policy snapshot.
...
export function planningHandoffUpdate(source: { id: string; version: number; metadata: Record<string, unknown> }, input: unknown) {
  ...
  return { id: source.id, expectedVersion: source.version, metadata: { ...source.metadata, planningHandoff: next,
