...
steps:
  ...
  - id: thumbnail
    ...
    completion:
      ...
      accepted_approval_statuses: [approved]
      result_status: approved
    work_action: 아이디어·AI 생성·AI 평가·사람 승인 폐쇄 루프 실행
    review_action: 승인 썸네일 업로드 후 CTR 학습까지 연결
    next_step: approval
  - id: approval
    order: 7
    label: 최종 승인
...
| 6 | thumbnail | AI+HUMAN | 아이디어 → AI 생성 → AI 평가 → 사람 승인 → 업로드 → CTR → 학습 |
...
`edit → thumbnail → approval → publish → metrics` 구간의 실행 규격은 `YOUTUBE_PIPELINE.json`이 상세화한다. PDF의 8개 후반작업 공정은 축약하지 않고 화면에 항상 표시하며 개인 PC에서 수행한다. 서버는 완료본 검증, 썸네일 생성·평가, 숏폼 생성, 게시, CTR 회수와 학습을 담당한다.
