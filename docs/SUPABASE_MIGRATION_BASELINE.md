...
The reviewed seven-file applied chain is recorded in the isolated `brandyaction-os-dev` project. Do not replay the schema baseline against Production. `supabase/migrations` contains the locally validated, schema-only snapshot of the Production `public` schema, the six already-applied forward migrations, the Production-approved update-quota patch, and one pending forward migration for separate folder-move limits and safe retry metadata. The 14 former delta files remain unchanged in `supabase/migrations-legacy` as historical evidence.

The user approved DEV application on 2026-09-17 with later reviewed forward migrations applied independently, and separately approved the relevant Production changes on 2026-09-21. Production already uses and records `20260921140000_agent_update_daily_limit.sql`, which raises only `knowledge.update` from 200 to 1,000 writes per rolling 24 hours. DEV has not applied that patch. The new `20260921233000_agent_write_rate_limit_metadata.sql` migration is code-reviewed work only and has not been applied to DEV or Production. It preserves the 1,000-write update fallback, separates folder-only moves, and exposes safe retry metadata. `npm run db:migrations:verify` remains the repository-integrity check; `db:migrations:ready` is intentionally false. This does not authorize applying either pending DEV migration, applying the new migration to Production, replaying the schema baseline, seeds, reset, Production data copy, or unrelated history repair.
