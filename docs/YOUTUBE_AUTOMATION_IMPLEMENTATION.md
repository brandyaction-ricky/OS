...
This code is closed unless `YOUTUBE_AUTOMATION_PILOT_ENABLED=true`, `OS_ENVIRONMENT=development` or `qa`, `NEXT_PUBLIC_DEMO_MODE=false`, and the configured DEV Supabase project reference exactly matches the server URL. Production project references must differ. On Vercel, only a Preview deployment from a non-main branch may pass the gate. To operate the voice stage, configure `YOUTUBE_SCENE_RULE_IDS`, server-only `OPENAI_API_KEY`, `FISH_API_KEY`, `FISH_VOICE_REFERENCE_ID`, optional `FISH_TTS_MODEL`, `SUPABASE_SERVICE_ROLE_KEY`, and `CRON_SECRET` in the verified DEV/Preview environment. Apply the additive private-bucket migration to DEV only after a separate environment review. A DEV scheduler must call `POST /api/v1/content/youtube-automation/worker` with `Authorization: Bearer <CRON_SECRET>` repeatedly; one call advances at most one step. No scheduler is registered in `vercel.json`, and the worker has not been run against a connected DEV environment. The existing Preview build alone does not prove voice synthesis works.

The branch still does not render a video, make a production thumbnail, produce timed captions, or upload to YouTube. The provisional photoreal-plus-graphics preview is a style direction; brand tone will be applied after the user reviews an actual video segment. Voice files are retained privately until a retention policy and cleanup worker are implemented, so connected use should use controlled pilot data.
