...
The private timing bucket migration `20260924010000_youtube_voice_timing.sql` was applied to isolated OS DEV on 2026-09-24 after user approval, in the same transaction as its migration history entry. Readback confirmed `public=false`, a 1 MiB limit, JSON-only MIME restriction, and one stored statement whose MD5 matches the committed migration file. Production was not changed. After a TypeScript narrowing fix (`d49b1c5`), Vercel Preview for the PR branch built successfully; this does not establish that the connected voice worker has run. The branch still does not render a video, make a production thumbnail, produce timed captions, or upload to YouTube. The local private preview demonstrates 25.6 seconds of content-only, speech-led character and diagram motion; it does not prove the connected OS renderer is implemented. `lib/server/youtube-timeline.ts` produces a render timeline only from verified final-audio word timestamps. The media worker still must assemble paragraph audio, verify each stored MP3 against its timing artifact, measure final duration and segment offsets, calculate final-audio SHA-256, and supply that transcript to this gate. A private character manifest and external media renderer remain to be connected. Final brand typography and colors remain adjustable. Voice files are retained privately until a retention policy and cleanup worker are implemented, so connected use should use controlled pilot data.
