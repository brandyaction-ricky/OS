...
- 원격 환경 점검: 로컬 checkout은 ignored `.vercel/project.json`으로 기존 프로젝트에 연결했다. 월 $0 비용을 확인·승인받아 서울 리전에 별도 `brandyaction-os-dev` 프로젝트를 생성했고, healthy 상태·public 테이블 0개·migration 0개·보안 advisor 0건을 확인했다. 저장소 migration 14개는 기존 core schema를 전제로 한 후속 delta이며 Production 이력 7개와 정확히 일치하는 식별자가 0개다. DEV migration 실행을 차단하고 파일 해시·필수 core contract·재현 절차를 `supabase/migration-baseline.json`과 `docs/SUPABASE_MIGRATION_BASELINE.md`에 고정했다. Production 데이터·스키마·설정은 변경하거나 복사하지 않았다. Vercel 환경변수 scope는 아직 설정하지 않았다.
- 남은 작업: 승인된 읽기 전용 Production schema export로 core baseline 생성, 로컬 zero-state 재구축과 advisor/RLS 검증, 별도 승인 후 DEV migration 적용, DEV/Preview Vercel 환경변수 scope 설정, 전용 테스트 계정으로 인증 브라우저 테스트 실행
