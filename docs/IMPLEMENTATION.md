...
| MCP | 구현 | Claude Code·Codex 검색·원문·개인 초안 생성·버전 수정·휴지통 이동 |
...
- Agent PAT는 서버에서 SHA-256 해시만 저장하며 `knowledge.read`와 선택적 `knowledge.write` 범위를 가진다.
- 쓰기 PAT는 반드시 활성 구성원과 조직 UUID에 귀속된다. 새 문서는 언제나 `draft`이며 다른 구성원의 비정본 문서는 수정할 수 없다.
- AI가 정본을 수정해도 기존 버전은 보존되고 에이전트명이 버전·`설정 → 감사 로그`에 기록된다.
- AI 삭제는 `archived` 휴지통 이동만 허용한다. 생성·수정은 분당 20건/24시간 200건, 삭제는 분당 5건/24시간 30건으로 제한한다.
