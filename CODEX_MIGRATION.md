...
The repository can be installed, tested, linted, type-checked, built, and run locally in demo mode. Repository CI performs the same side-effect-free gate plus a browser smoke test. The local checkout is linked to the existing Vercel project. A dedicated empty Supabase DEV project has been provisioned. QA is a Preview verification stage using isolated DEV resources; Vercel environment-variable scope has not been established.
