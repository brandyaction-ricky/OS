...
OS는 직원 PC의 파일이나 개인 AI 계정에 접근하지 않는다. PDF의 8개 후반작업 공정은 전체 실행공정 안에서 각각 독립적으로 활성화되며, 사용자는 단계별 개인 PC 자동화 결과 또는 판단을 기록한 뒤 최종 MP4·SRT를 회사 자산 저장소로 인계한다. 썸네일은 OS의 별도 폐쇄 루프에서 생성·평가·승인·업로드·측정·학습한다.
...
상위 구분은 탐색을 위한 폴더일 뿐 공정을 대체하지 않는다. 왼쪽 전체 실행공정에는 총 20개 실행 Stage를 항상 표시한다. 다음 PDF 원본 8공정도 하나의 상세 카드에 묶지 않고 각각 고유한 상태·선행조건·입력·산출물·자동화 실행 또는 사람 판단 화면을 갖는다.
...
선택한 공정 화면에는 그 공정에 필요한 설명·도구·입력·산출물·실행 설정과 관련 절대 규칙만 표시한다. 기술 입력·출력 계약과 Worker 로그는 현재 Stage 아래에서 펼쳐볼 수 있게 유지한다.
