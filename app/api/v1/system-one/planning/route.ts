...
import { checkPlanningReferences, planningRegistryConfig } from "@/lib/server/system-one-planning";
...
export async function GET(request: Request) {
  ...
  try {
    const result = await checkPlanningReferences({ id: query.get("id"), expectedVersion: Number(query.get("version")) }, registry,
      { content: createSystemOneUserContentSource(request), documents: createSystemOneUserDocumentSource(request) });
    ...
    return NextResponse.json({ status: "ready", policyStatus: "unverified", judgment: null, executionAllowed: false,
      source: { id: result.source.id, version: result.source.version },
      registryVersion: result.registryVersion, referenceCount: result.referenceCount }, { headers });
