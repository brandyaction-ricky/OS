...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ plan: buildYoutubeAutomationPlan(state), voicePreviewConfigured: Boolean(process.env.FISH_API_KEY?.trim() && process.env.FISH_VOICE_REFERENCE_ID?.trim()) }, { headers });
