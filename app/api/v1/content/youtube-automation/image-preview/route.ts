...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { state, plan } = await readYoutubeAutomationInput(actor.supabase, input.sourceId);
    if (state.source.owner_id !== actor.id) throw new ApiError(403, "CONTENT_OWNER_REQUIRED", "이 콘텐츠의 소유자만 이미지를 생성할 수 있습니다.");
    const scenePlan = state.source.metadata.narratedScenePlan as { inputKey?: string; ruleVersions?: unknown; templateVersion?: unknown; plan?: unknown } | undefined;
    ...
    if (!plan.inputKey || plan.inputKey !== input.inputKey || scenePlan?.inputKey !== input.inputKey ||
      scenePlan.templateVersion !== YOUTUBE_VISUAL_TEMPLATE_VERSION || !scene)
    ...
    const latestScenePlan = latest.source.metadata.narratedScenePlan as { inputKey?: unknown; templateVersion?: unknown } | undefined;
    if (latestPlan.inputKey !== input.inputKey || latestScenePlan?.inputKey !== input.inputKey ||
      latestScenePlan.templateVersion !== YOUTUBE_VISUAL_TEMPLATE_VERSION ||
      JSON.stringify((await readYoutubeSceneRules(actor)).ruleVersions) !== ruleVersions)
