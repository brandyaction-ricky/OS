...
- A dedicated Supabase DEV project exists in Seoul. The repository now contains one core baseline plus 14 forward migrations. The original four-file zero-state validation is bootstrap evidence only; later forward migrations were applied individually under separate approvals. Four content-evidence and archived-document migrations are DEV-only and have no Production authorization. QA uses an immutable Preview commit against isolated DEV resources; a separate QA database is not required. Production must not be used for connected local or QA tests.
- The eight core Vercel variables are scoped only to Preview and Development. Production values were not changed.
- A schema-only Production snapshot is the single active baseline; the 14 prior deltas are frozen outside the active chain. The last recorded Production history inventory is time-bound and showed non-matching entries. Read `supabase/migration-baseline.json` together with the dated evidence in `docs/SUPABASE_MIGRATION_BASELINE.md`; neither a successful migration check nor an old `ready`/`apply` value authorizes Production changes. Freshly inspect the intended environment before every candidate.
